...
async fn handle_client(mut stream: TcpStream) -> Result<(), Box<dyn Error>> {
    ...
    let mod_path = format!("./challenge/build/uctt/bytecode_modules/{}.mv", modules[0]);
    ...
    let mut object_output = apt_ctf_framework::view_object(&mut adapter, owner_address, &module_id, &ident_str!("Debug"), Vec::new());
    println!("Object Output: {:#?}", object_output);

    // Check SafeDepositBox (View Object)
    let mut owner_address = AccountAddress::from_hex_literal("0x5d26592cd1c87c51aec9a4f0071011905b534b62a0eae4c5966ef8f13b5f4011").unwrap();
    let mut module_id: ModuleId = ModuleId::new(chall_addr, Identifier::new(modules[0]).unwrap());
    let mut object_output = apt_ctf_framework::view_object(&mut adapter, owner_address, &module_id, &ident_str!("SafeDepositBox"), Vec::new());
    println!("Object Output: {:#?}", object_output);
    ...
    // Check Solution
    let mut args2_solve: Vec<MoveValue> = Vec::new();
    args2_solve.push(MoveValue::Address(AccountAddress::from_hex_literal("0x9c3b634ac05d0af393e0f93b9b19b61e7cac1c519f566276aa0c6fd15dac12aa").unwrap()));
    let type_args : Vec<TypeTag> = Vec::new();

    // Call touched_this Function
