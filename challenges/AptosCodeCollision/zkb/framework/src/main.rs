...
async fn handle_client(mut stream: TcpStream) -> Result<(), Box<dyn Error>> {
    ...
    let mod_path = format!("./challenge/build/zkb/bytecode_modules/{}.mv", modules[0]);
    ...
    let mut object_output = apt_ctf_framework::view_object(&mut adapter, owner_address, &module_id, &ident_str!("ProofStatus"), Vec::new());
    println!("Object Output: {:#?}", object_output);

    for _ in 0..3 {

        let random_number: u64 = rand::thread_rng().gen::<u64>() % 0xff;
        println!("Random number: {:#04x}", random_number);

        // Call set_knowledge Function
        let mut args_solve: Vec<MoveValue> = Vec::new();
        args_solve.push(MoveValue::U64(random_number));
        type_args = Vec::new();
        ...
        object_output = apt_ctf_framework::view_object(&mut adapter, owner_address, &module_id, &ident_str!("Knowledge"), Vec::new());
        println!("Object Output: {:#?}", object_output);
