...
async fn handle_client(mut stream: TcpStream) -> Result<(), Box<dyn Error>> {
    ...
    loop {
        ...
        if let Some((address, module, function)) = parse_function_to_invoke(&function_to_invoke) {
            ...
            if module == "exploit" && function == "admin_deposit" {
                // admin_deposit
                admin_call_function(
                    &mut adapter,
                    chall_addr,
    ...
    // Check Solution
    let sol_ret = call_function(&mut adapter, chall_addr, "swap", "is_solved", "challenger");
...
fn admin_call_function(
    ...
    let args: Vec<MoveValue> = vec![MoveValue::U64(8)];
    let type_args = Vec::new();
    apt_ctf_framework::call_function(
        adapter,
        addr,
